...
```text
thread_pool_logger/
├── CMakeLists.txt        # CMake automated build configuration file
├── include/              # Header files directory (exposed interface declarations)
│   ├── AsyncLogger.h     # Async logger module declaration
│   ├── BlockingQueue.h   # Thread-safe blocking queue template implementation
│   └── ThreadPool.h      # Thread pool module declaration
├── src/                  # Source code directory (core module implementations)
│   ├── AsyncLogger.cpp   # Async logger function implementation
│   ├── ThreadPool.cpp    # Thread pool function implementation
